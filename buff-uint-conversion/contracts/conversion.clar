...
;; (contract-call? .conversion uint-to-buff u5233100606242806050955395731361295)
(define-public (uint-to-buff (n uint))
    (if (is-eq n u0) (ok 0x00)
    (begin 
        (var-set out-buf 0x)
        (var-set so-far n)
        (map mod-256 mods)
...
;; (contract-call? .conversion buff-to-uint 0x000102030405060708090A0B0C0D0E0F)
